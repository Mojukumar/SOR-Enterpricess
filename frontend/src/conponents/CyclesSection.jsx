...
const CyclesSection = ({ data }) => {
  const [cycleData, setCycleData] = useState(data);
  ...
  const handleUpdate = (id) => {
    ...
    // Find the cycle to be updated based on its ID
    const cycleToUpdate = cycleData.find(item => item._id === id);
    setUpdatedItem(cycleToUpdate);
  };
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      // Make an API request to update the cycle with the new information
      await axios.put(`http://localhost:5000/api/v3/updateCycle/${updatedItem._id}`, updatedItem);
  ...
  const handleDelete = async (id) => {
    ...
    setCycleData((prevData) => prevData.filter(item => item._id !== id));

    try {
      await axios.delete(`http://localhost:5000/api/v3/deleteCycle/${id}`);
      ...
      setCycleData((prevData) => [...prevData, ...cycleData.filter(item => item._id === id)]);
    }
  };
